...
def _preprocess_track3_code(
    ...
    additional_charts_path : str
        Relative file path where we are saving the additional charts.
    Returns
    -------
    str
    ...
    full_code_block += "import boto3\n"
    full_code_block += "from io import StringIO\n"
    full_code_block += "import os\n"
    full_code_block += "import posixpath as pp\n"
    full_code_block += "import s3fs\n"
    full_code_block += "\n"
...
if cloud_config.cloud_provider != "s3":
    if(prefix_url is None and storage_options is None):
    ...
    output_s3_path = "{additional_charts_path}"
    """
    ...
    if cloud_config.cloud_provider != "s3":
        full_code_block += f"""df = pd.read_csv(prefix_url+r"{input_file_path}", storage_options=storage_options)"""
    else:
        full_code_block += """df = pd.read_csv(csv_data)"""
    full_code_block += "\n"
    ...
    if fig_show_str_count > 0:
        if cloud_config.cloud_provider != "s3":
            repl_strings = [
                f"""with fs.open("{pp.join(additional_charts_path, f"additional_charts_{i+1}.png")}", mode="wb", newline="") as fp:
                    plt.savefig(fp)"""
                for i in range(fig_show_str_count)
            ]
        else:
            repl_strings = [
                f"""with fs.open("s3://{cloud_config.domain_storage.account_name}/{additional_charts_path}/additional_charts_{i+1}.png", mode="wb") as fp:
            plt.savefig(fp)"""
                for i in range(fig_show_str_count)
            ]
        full_code_block = re.sub(
            pattern="plt\.show\(.+",
            repl=lambda repl: repl_strings.pop(0),
            string=full_code_block,
        )
    print("--------------------------------------full_code_block", full_code_block)
...
def _run_full_code(
    ...
    try:
        ...
        logger.debug(
            f"Insights full code run return status = {subprocess_result.returncode}"
        )
...
def run_insights_code(
    ...
    full_code_str = _preprocess_track3_code(
        ...
        additional_charts_path=pp.join(track3_path, "additional_charts"),
    )

    print(full_code_str, "track 3 post processed **************************")

    # Write the code to python file as per folder structure.
    logger.info("Saving Track 3 code.")

    # Format the code using Black.
    full_code_str = black.format_str(full_code_str, mode=black.Mode())

    # Format the code using isort.
    full_code_str = isort.code(full_code_str, float_to_top=True)
    ...
    if entire_code_status:
        ...
        code_blocks = []
        for i in range(0, len(split_point_linenum), 2):
            if i == len(split_point_linenum) - 1:
                block = code_at_line_level[0 : split_point_linenum[0]]
                block += code_at_line_level[split_point_linenum[i] :]
                block = "\n".join(block)
            else:
                block = code_at_line_level[0 : split_point_linenum[0]]
                block += code_at_line_level[
                    split_point_linenum[i] : split_point_linenum[i + 1]
                ]
        ...
        for i, blk in enumerate(code_blocks):
            ...
            code_block_fname = pp.join(track3_path, "code_blocks", f"code_block_{i}.py")
...
def extract_code(string_input: str, start: list, end: str, extract: str):
    ...
    if len(end) > 1:
        # If there are more than one ending patterns, replace all other patterns with the first pattern.
        for i in range(1, len(end)):
            string_input = string_input.replace(end[i], end[0])
...
if cloud_config.cloud_provider != "s3":
    ...
    csv_string = response['Body'].read().decode('utf-8')
    csv_data = StringIO(csv_string)
    chart_output_key = "{chart_save_path}"
    ...
    figure_declaration_match = re.search(
        r"(\w+)\s*=\s*go\.Figure\(.*\)", full_code_block
    )

    # Regular expression to search for fig.update_layout() and fetch the variable name
    update_layout_variable_match = re.search(
        r"(\w+)\s*\.update_layout\(", full_code_block
    )
    ...
    if "import plotly.io" in full_code_block:
        if re.search(
            pattern=r"\w+\.show\((\w+)\)", string=full_code_block, flags=re.MULTILINE
        ):
            plotly_figure = re.search(
                pattern="\w+\.show\((\w+)\)", string=full_code_block
            ).group(1)
            full_code_block = re.sub(
    ...
    with fs.open("{chart_save_path}", mode="w") as fp:"""
        ...
        plotly.io.write_json({plotly_figure}, file=fp, pretty=True)
                ...
                save_fig_line = (
                    rf"plotly.io.write_json({plotly_figure}, json_buffer, pretty=True)"
                ) + "\n"
                save_fig_line += """json_str = json_buffer.getvalue()""" + "\n"
                save_fig_line += "s3.put_object(Bucket=bucket_name, Key=chart_output_key, Body=json_str)"
    ...
    with fs.open("{chart_save_path}", mode="w") as fp:"""
        ...
        if search_pattr:
            ...
            if cloud_config.cloud_provider != "s3":
                save_fig_line = (
                    rf"""with fs.open("{chart_save_path}", mode="w") as fp:"""
                )
                save_fig_line += (
                    rf"""plotly.io.write_json({plotly_figure}, file=fp, pretty=True)"""
                    + "\n"
                )
            else:
                save_fig_line = (
            ...
            if os.name == "nt":
                indented_replacement = indented_replacement.encode(
                    "unicode-escape"
                ).decode()
...
def add_exception_to_code(
    ...
    for comment, line in lines:
        if not line[0].startswith("    ") and "pd.DataFrame(" not in line[0]:
            if line[0].startswith((include_pattern)) and not line[0].startswith(
                (exclude_pattern)
            ):
...
def _detect_alter_drop_table(query):
    ...
    pattern = re.compile(
        r"(ALTER TABLE|DROP TABLE|CREATE TABLE|INSERT INTO)", re.IGNORECASE
    )
...
def _complete_data_dict(output_table, raw_dict: dict, result_dict: list):
    ...
    for _, dict_ in raw_dict.items():
        if dict_ is not None and "columns" in dict_:
            raw_dict_list_temp += dict_["columns"]

    # rename all the "name" with "column_name"
    raw_dict_list = []
    for e in raw_dict_list_temp:
        raw_dict_list.append(
            {"column_name": e["name"], "description": e["description"]}
        )
...
def check_for_static_lists(code: str) -> bool:
    """Check if the code contains static numeric lists. Used to identify when chart code generated by the LLM contains hallucinated sample data in static numeric list format
    ...
    -------
    bool
        Flag if the code contains static numeric lists created then returns True else returns False
    """
    tree = ast.parse(code)
    ...
    class ListVisitor(ast.NodeVisitor):
        ...
        def visit_List(self, node):
            if isinstance(node.parent, ast.Assign):
                if isinstance(node.parent.value, ast.List):
                    # Check if it is a non-empty list
                    if node.parent.value.elts:
                        # Check if all elements of the list are numeric
                        if all(
                            isinstance(element, (ast.Constant, ast.Num))
                            and isinstance(element.value, (int, float))
                            for element in node.parent.value.elts
                        ):
                            # Check if the list is assigned to a variable
                            if isinstance(node.parent.targets[0], ast.Name):
                                self.static_lists.append(node.parent.targets[0].id)
            self.generic_visit(node)

    visitor = ListVisitor()
    visitor.visit(tree)

    if visitor.static_lists:
        return True
    else:
        return False
