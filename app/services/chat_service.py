...
from core.utils.read_config import (app_database_config, cloud_config,
                                    cloud_secrets, config, secrets_config, deployment_url_config)
...
def get_report_list(user_id: str, data_name: str, db_conn=None, client=None):
    ...
    """
    try:
        logging.info(f"Fetching chat reports for user : {user_id}")
        ...
        result = []
        if not report_rows:
            logging.info(f"No chat reports found for user : {user_id}")
        else:
        ...
        logging.error(traceback.format_exc())
        print(e)
        return [] # forcing a empty list return to hadle the front end bug.(Bug: None object cannot be iterated.)
...
def check_existing_user(user_id: str, data_name: str, db_conn=None, client=None):
    ...
    try:
        ...
        query = f"SELECT * FROM user_config WHERE user_id like lower('{user_id}') and lower(data_name) like (lower('{data_name}'));"
        ...
        if len(user_rows) > 0:
            ...
            logging.info(f"User ID : '{user_id}' found. Updating ast login time.")
            ...
            # get chat details
            query = f"""SELECT cd.chat_id,
                               cd.chat_name,
                               cd.is_active,
                               cc.context_id,
                               cc.data_name
                        FROM chat_details AS cd
                             LEFT JOIN chat_contexts AS cc 
                             ON cc.chat_id = cd.chat_id AND
                             lower(cc.data_name)=lower(cd.data_name)
                        WHERE cd.user_id = '{user_id}' AND 
                              lower(cd.data_name) = lower('{data_name}') AND
                              cd.is_active = TRUE AND
                              cc.is_active = TRUE;"""
            chat_details_row = client.read_query(connection=db_conn, query=query)
            # print(f'here   {chat_details_row}')
            if chat_details_row:
                result.chat_details.chat_id = chat_details_row[0][0]
                result.chat_details.data_name = chat_details_row[0][4]
                result.chat_details.chat_name = chat_details_row[0][1]
                result.chat_details.is_active = chat_details_row[0][2]
                result.chat_details.context_id = chat_details_row[0][3]
                logging.info(f"data name : {chat_details_row[0][4]}")
                if result.chat_details.data_name is None:
                    logging.info(f"Received Null data_name from DB. assiging data name :{data_name} as received.")
                    result.chat_details.data_name = data_name #sometimes dataname is returned as null. hardcoding to dataname sent by the front end
                # Get report
                result.report_list = get_report_list(
                    user_id=user_id, data_name=data_name, db_conn=db_conn, client=client
                )
                ...
                result.chat_details.chat_history = get_chat_history_by_chat_id(
                    chat_ids=[result.chat_details.chat_id],
                    data_names=[result.chat_details.data_name],
                    db_conn=db_conn,
                    client=client,
                )
                for i, j in result.chat_details.chat_history.items():
                    for v in j:
                        if "data" in v and "content" in v["data"]:
                            v["data"] = v["data"]["content"]
            else:   #this case is hypothetical. User had conversed, but there are no records. Silly guy.
                result = reset_chat(user_id=user_id, data_name=data_name, db_conn=db_conn, client=client)

        else:
            # Functionality for New user
            logging.info("User '{user_id}' not found. Creating a record. ")
...
def create_new_question(
    ...
    try:
        ...
        data = {
            "user_id": user_id,
            "question_id": content[0]["question_id"],
            "answer_id": content[0]["answer_id"],
            "question": question,
        ...
        if response.status_code == 200:
            ...
            content_track1 = output_response
        else:
        ...
        if response.status_code == 200:
            ...
            content_track2 = output_response
        else:
        ...
        if response.status_code == 200:
            ...
            content_track3 = output_response
        else:
        ...
        data = {
            "user_id": user_id,
            "question_id": content_track3["question_id"],
            "answer_id": content_track3["answer_id"],
            "context_id": context_id,
            "chat_id": chat_id,
            "content": content_track3,
            "question": question,
            "data_name": data_name,
            "track_1": content_track1,
            "track_2": content_track2,
            "track_3": content_track3,
        }
        # Post Track
        post_track_api = url + "post_track"
        # Make the POST request
        # logging.info("\nPost Track API Process started")
...
def get_questions_by_context_id(context_id: int, db_conn=None, client=None):
    ...
    try:
        ...
        response_list = db_factory.read_query(connection=db_conn, query=query)
...
def check_user_type_request(user_id, context_id, question, db_conn=None, client=None):
    """
    ...
    if user_type == "external":
        remainder = check_reqs_limit(user_id=user_id, db_conn=db_conn,client=client)
        if remainder is None:
            return None, "Exhausted Questions Quota!!"
    ques_list, botlist = get_questions_by_context_id(context_id, db_conn,client)
...
def json_format(dict_file):
    dict_file = (
        json.dumps(dict_file)
        .replace("\n", " ")
        .replace("'", " ")
    )
    # dict_file = json.dumps(dict_file)
    return dict_file


def db_storage(user_id, question_id, answer_id, context_id, chat_id, content, question, data_name, db_conn, track_1, track_2, track_3):
    """
    ...
    content = copy.deepcopy(track_1)
    # logging.info(content["Response JSON"])
    if track_2 is not None:
        content["content"]["data"].append(track_2["content"]["data"][1])
    if 'skip' not  in track_3["content"]['status']:
        content["content"]["data"].append(track_3["content"]["data"][1])
        content["content"]["completion_response"] = track_3["content"]["completion_response"]
        content["content"]["response_for_history"] = track_3["content"]["response_for_history"]
    content_combined = (
        json.dumps(content)
        .replace("\n", " ")
        .replace("'", " ")
    )
    category = track_1["category"]
    track_1 = json_format(track_1)
    track_2 = json_format(track_2)
    track_3 = json_format(track_3)

    query = f"""
        UPDATE chat_responses
        set
        category = '{category}', created_time=CURRENT_TIMESTAMP, modified_time=CURRENT_TIMESTAMP, content = '{content_combined}', track1_res='{track_1}',
        track2_res='{track_2}', track3_res='{track_3}'
        where answer_id={answer_id};
    """
    # TODO
    update_response = db_factory.execute_query(db_conn, query, False)
    logging.info("Updated database with chat response.")

    logging.info("update_response %s", update_response)
    question_to_insert = question.replace("'", "''")
    response_to_insert = content["content"]["response_for_history"].replace(
        "'", "''"
    )
    question_index = content["content"]["question_index"]
    output_path = content["content"]["output_path"]
    engines = json.dumps(content["content"]["engines"])
    # store question data and answer_id in chat_questions
    query = f"""UPDATE chat_questions
                set
                user_question='{question_to_insert}', answer_id={answer_id}, context_id={context_id}, chat_id={chat_id},
                user_id='{user_id}', created_time=CURRENT_TIMESTAMP, modified_time=CURRENT_TIMESTAMP, response_for_history='{response_to_insert}',
                data_name='{data_name}', question_index='{question_index}', output_path='{output_path}', engines='{engines}' 
                where question_id={question_id};
            """
    update_data = db_factory.execute_query(db_conn, query, False)
    logging.info("Updated database with chat questions.")
    logging.info("update_data %s", update_data)
...
def response_format(user_id, question_id, answer_id, response_json, question_path):
    output = {}
    output["error"] = None
    output["user_id"] = user_id
    output["question_id"] = question_id
    output["answer_id"] = answer_id
    output["category"] = response_json["Response JSON"]["type"]
    output["content"] = response_json["Response JSON"]
    output["question_path"] = question_path
