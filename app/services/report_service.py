...
def create_new_report(
    ...
    """
    try:
        logging.info(f"Genarating report for the user : {user_id}")
        ...
        logging.info(f"Fetching chat details")
        (
        ...
        logging.info(f"Generated Report File Name : {file_name}")

        logging.info(f"Uploading the report.")
        ...
        logging.error(f"Exception occured in the report generation. \n ERROR : \n\n {e}")
...
def get_details_by_chat_id(chat_id: int, data_name: str, db_conn=None, client=None):
    ...
    res = client.read_query(connection=db_conn, query=query)
...
def create_entry(
    ...
    for i, entry in enumerate(content_list):
        ...
        for content in entry["content"]["data"]:
