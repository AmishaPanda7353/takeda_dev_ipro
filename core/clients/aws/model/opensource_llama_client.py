...
class OpenSourceLlamaModelCall(Model):
    """
    ...
    def model_response(
        ...
        if api_type == "aws_llama":

            if history is not None:
                self.prompt = self.prompt + "\n\n"
                self.prompt += f"llama response:\n\n{history}\n\n"
                self.prompt += f"New Question: {debug_prompt}"

            body = json.dumps(
                {
                    "prompt": self.prompt,
                    "max_gen_len": model_param_dict.max_gen_len,
                    "top_p": model_param_dict.top_p,
            ...
            #     "generation_token_count": response["generation_token_count"],
            # }
            # tokens = None
